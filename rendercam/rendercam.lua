...
M.view = {[hash("/camera_3d")] = vmath.matrix4(), [hash("/camera_map")] = vmath.matrix4()}
M.proj = {[hash("/camera_3d")] = vmath.matrix4(), [hash("/camera_map")] = vmath.matrix4()}
...
function M.calculate_view(cam) -- called from render script on update
	...
	M.view[cam] = vmath.matrix4_look_at(cameras[cam].wpos, cameras[cam].wpos + cameras[cam].wforwardVec, cameras[cam].wupVec)
	return M.view[cam]
end

function M.calculate_proj(cam) -- called from render script on update
	if cameras[cam].orthographic then
		local x = cameras[cam].halfViewArea.x * cameras[cam].orthoScale
		local y = cameras[cam].halfViewArea.y * cameras[cam].orthoScale
		M.proj[cam] = vmath.matrix4_orthographic(-x, x, -y, y, cameras[cam].nearZ, cameras[cam].farZ)
	else -- perspective
		M.proj[cam] = vmath.matrix4_perspective(cameras[cam].fov, cameras[cam].aspectRatio, cameras[cam].nearZ, cameras[cam].farZ)
	end
	return M.proj[cam]
end
...
function M.screen_to_viewport(x, y, delta)
	...
	if delta then
		...
		x = (x - viewport_main.x) / 1 --M.viewport.scale.x
		y = (y - viewport_main.y) / 1 --M.viewport.scale.y
	end
	return x, y
end
...
function M.screen_to_world_plane(x, y, planeNormal, pointOnPlane, cam)
	local np, fp = M.screen_to_world_ray(x, y, nil, cam)
...
function M.world_to_screen(pos, adjust, raw, cam)
	...
	pv = m * pv
	pv = pv * (1/pv.w)
	pv.x = (pv.x / 2 + 0.5) * viewport_main.width + viewport_main.x
	pv.y = (pv.y / 2 + 0.5) * viewport_main.height + viewport_main.y
